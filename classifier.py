...
# 长词不分大小写
_INDEX_ANY = re.compile(r"(?i)\b(nasdaq|s&p\s?500|s&p|sp500|spy|qqq|dow|djia|russell|vix|futures?)\b")
...
LEVEL_MIN, LEVEL_MAX = 300.0, 60_000.0   # SPY ~600 到 NQ ~26,000+，留余量
...
def _has_index_keyword(text: str) -> bool:
    if _INDEX_UPPER.search(text) or _INDEX_ANY.search(text):
        return True
